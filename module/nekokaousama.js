...
async function ousama(messageId, roomId, accountId) {
  ...
  const message = `王様は[To:${kingAccountId}][pname:${kingAccountId}]さんにゃ。番号は1〜${
...
async function Participant_display(roomId) {
  try {
    ...
    let Id = `[info][title]登録されてる人にゃ　${canExit}　${shuffle}[/title]`;
    const data = await readFileAsync("participant");
    Id += data;
    Id += "[/info]";
    await sendchatwork(Id, roomId);

    let name = `[info][title]登録されてる人にゃ　${canExit}　${shuffle}[/title][piconname:`;
...
async function facilitator(body, messageId, roomId, accountId) {
  try {
    ...
    await sendchatwork(
      `[rp aid=${accountId} to=${roomId}-${messageId}][pname:${accountId}]\n進行役を[piconname:${facilitator}]さんに設定しましたにゃ`,
      roomId
    );
  } catch (error) {
    console.error(error);
...
async function facilitator_display(roomId) {
  try {
    const data = await readFileAsync("facilitator");
    await sendchatwork(
      `[info][title]進行役にゃ[/title][piconname:${data}][/info]`,
      roomId
    );
  } catch (error) {
    console.error(error);
...
async function systemToggle(roomId) {
  try {
    ...
    if (system == "起動中") {
      ...
      await sendchatwork("システムを全停止しますにゃ", roomId);
    } else {
